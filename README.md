...
# 克隆特定 skill（以 milestone-builder 为例）
git clone https://github.com/Aaron-cdx/ei-skill.git
cp -r ei-skill/milestone-builder .claude/skills/
rm -rf ei-skill
```
...
```bash
mkdir -p .claude/skills
git clone https://github.com/Aaron-cdx/ei-skill.git .claude/skills/ei-skill

# 然后将各子目录软链接或复制到 .claude/skills/ 下
cp -r .claude/skills/ei-skill/milestone-builder .claude/skills/
```
